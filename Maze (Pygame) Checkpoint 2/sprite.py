...
class NPCSprite(Sprite):
    ...
    # Behavour of NPC to wander aimlessly while swimming
    def wander_swim(self):
        tilemap = self.level.tilemap
        colliders = self.level.colliders
        if self.cycle > 384:
            self.cycle = 0
        ...
        if self.cycle <= 80:
            ...
            self.frame = 0
        NPCSprite.simple_update_frame(self)
    ...
    # Determine which of four frames should be shown
    def simple_update_frame(self):
        if self.facing == "front":
            if self.direction == None:
                self.sprite = self.images["front"]
            else:
                self.sprite = self.images[self.direction]
        else:
            if self.direction == None:
