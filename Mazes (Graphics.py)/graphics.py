...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        self.bind_all("<KeyPress>", self._pressed)
        self.bind_all("<KeyRelease>", self._released)
        ...
        self.trans = self.trans = Transform(self.width, self.height, 0, self.height, self.width, 0)
        self.closed = False
        master.lift()
        self.lastKey = ""
        self.keys = {'d': False, 'w': False, 'a': False, 's': False, 'escape': False, 'shift_l': False}
    ...
    def _pressed(self, evnt):
        self.keys[evnt.keysym.lower()] = True
        self.lastKey = evnt.keysym

    def _released(self, evnt):
        self.keys[evnt.keysym.lower()] = False

    def set_icon(self, icon):
        '''This function was added to the graphics.py module to allow the window's icon to be set.\n
        the file path must be a complete path.  Note to self: look into changing this in the future.'''
        img = tk.Image("photo", file=icon)
        self.master.tk.call('wm', 'iconphoto', self.master._w, img)

    def set_fullscreen(self, state):
        '''This function was added to graphic.py to allow the window to toggle fullscreen'''
        self.master.attributes("-fullscreen", state)
        self.fullscreen = state

    def get_screen(self):
        '''This function was added to the module to get the screen size'''
        return self.master.winfo_screenwidth(), self.master.winfo_screenheight()

    def set_size(self, dx, dy):
        '''This function was added to the module to change the window size'''
        self.width = dx
        self.height = dy
        self.pack(fill="both", expand=True)

    def toggle_fullscreen(self):
        '''This function was added to the module to make it easier to toggle fullscreen'''
        self.fullscreen = not(self.fullscreen)
        self.set_fullscreen(self.fullscreen)
        if self.fullscreen:
           screen_size = self.get_screen()
           self.set_size(screen_size[0], screen_size[1])
        else:
           self.set_size(self.base_width, self.base_height)
        self.setCoords(0, self.base_height, self.base_width, 0)

    def set_shift(self, x, y):
        '''Function added to module to allow for shifting of entire screen'''
...
class Image(GraphicsObject):
    ...
    def _draw(self, canvas, options):
        #print(canvas.trans.xscale)
        p = self.anchor
        x,y = canvas.toScreen(p.x,p.y)
        self.imageCache[self.imageId] = self.img # save a reference
        self.images[self.imageId] = self
        #new_size, new_size = canvas.toScreen(self.img.width(), self.img.height())
        #self.img = self.rescale(new_size)
    ...
    def rescale(self, new_size):
        img = self.img.zoom(new_size)
        return img.subsample(self.img.width())
