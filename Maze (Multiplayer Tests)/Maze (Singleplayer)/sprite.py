...
class Level:
    ...
    def __init__(self, colliders, tilemap, decomap, loading_zones, lightmap=[], default_start=(0, 0), name="Unnamed"):
        ...
        if name in Level.reference_dict:
            print("WARNING: Multiple levels have the same name '{}'".format(name))
        self.name = name
...
class Sprite:
    ...
    def check_collision(self, dx, dy, tilemap, collider_ids):
        ...
        try:
            ...
            if tilemap[y2][x2] in collider_ids:
                return False
            if Sprite.check_sprites_collide(self, dx, dy) is not None:
                return False
            return True
        except IndexError:
            return False
    ...
    def behave_all(cls):
        ...
        for s in cls.global_sprites:
            if type(s) == PlayerSprite and s.check_load_zone():
...
Behavour: {}
Action: {}
Action Arguments: {}""".format(self.rect, self.behavour, self.action, self.action_args)
        try:
