...
class Game:
    ...
    def wipe_effect(self):
        ...
        self.win2.fill((0, 0, 0))
        self.render()
        wipe_down = pygame.image.load(os.path.join('assets/wipe_down.png')).convert_alpha()
        self.wipe_basic(wipe_down, self.win2.copy(), 0)
        #self.tile_unwipe()

    def tile_unwipe(self):
        window = self.win2
        litmap = self.litmap
        level = self.focus.level
        dispx, dispy = center_at(self.focus.x, self.focus.y, self.focus.level.width, self.focus.level.height)
        clock = pygame.time.Clock()
        x_tile = int(abs(dispx / 64))
        y_tile = int(abs(dispy / 64))
        y = y_tile - 1
        for i, k in zip(level.tilemap[y_tile:y_tile + 10], level.decomap[y_tile:y_tile + 10]):
            y += 1
            x = x_tile - 1
            for j, m in zip(i[x_tile:x_tile + 17], k[x_tile:x_tile + 17]):
                x += 1
                
                if j in level.tile_ids:
                    if litmap[y][x]:
                        level.tile_ids[j].draw_norm(window, (dispx + x * 64, dispy + y * 64))
                    else:
                        level.tile_ids[j].draw(window, (dispx + x * 64, dispy + y * 64))
                        
                if m in level.deco_ids:
                    if litmap[y][x]:
                        level.deco_ids[m].draw_norm(window, (dispx + x * 64, dispy + y * 64))
                    else:
                        level.deco_ids[m].draw(window, (dispx + x * 64, dispy + y * 64))
                pygame.transform.scale(window, (Game.ext_width, Game.ext_height), Game.win)
                pygame.display.flip()
            clock.tick(20)

        incomplete_img = window.copy()
        entity_img = (window.copy()).convert_alpha()
        entity_img.fill((0, 0, 0, 0))
        Sprite.blit_all(dispx, dispy, entity_img)
        entity_img_copy = entity_img.copy()

        window.fill((0, 0, 0))
        
        for i in range(8):
            window.blit(incomplete_img, (0, 0))
            entity_img.blit(entity_img_copy, (0, 0))
            entity_img.fill((255 - 32 * i, 255 - 32 * i, 255 - 32 * i), special_flags=pygame.BLEND_ADD)
            window.blit(entity_img, (0, 0))
            pygame.transform.scale(window, (Game.ext_width, Game.ext_height), Game.win)
            pygame.display.flip()
            clock.tick(60)                        
    ...
    def play(self):
        ...
        while not done:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_BACKSPACE:
                        ...
                        if self.full:
                            Game.win = pygame.display.set_mode((Game.screen_width, Game.screen_height), pygame.FULLSCREEN | pygame.DOUBLEBUF)
            ...
            # Update entities
            Sprite.behavour_args['keys'] = pygame.key.get_pressed()
            ...
            if debug: step_list.append(time_step)
